...
function InternetMobilePageContent() {
  const router = useRouter();
  ...
  const [activeCategory, setActiveCategory] = useState("internet-mobile");
  ...
  const handleCategoryChange = (categoryId: string) => {
    ...
    if (categoryId === "all") {
      // При выборе "Все" устанавливаем фильтры для интернет+мобильная связь
      setFilters(prev => ({
        ...prev,
        internet: true,
        tv: false,
      ...
      <div style={{background: "linear-gradient(90deg, #F26A2E 0%, #7B2FF2 100%)", padding: "20px 0 32px 0", color: "#fff"}} className="md:py-8 md:pb-12">
        <div style={{maxWidth: 1200, margin: "0 auto", padding: "0 16px"}}>
          <div style={{fontSize: 14, opacity: 0.8, marginBottom: 12}} className="md:text-base md:mb-4">
            Ростелеком / {city} / <b>Интернет + Моб. связь</b>
          </div>
          <h1 style={{fontSize: 28, fontWeight: 700, lineHeight: 1.1}} className="md:text-5xl">
            Тарифы Ростелеком на интернет и мобильную связь в {city}
          </h1>
        </div>
      </div>
