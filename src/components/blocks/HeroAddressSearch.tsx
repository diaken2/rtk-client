  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Можно показать ошибку пользователю, но пока просто переходим
        router.push('/complete');
      }
    } catch (error) {
      console.error('Error submitting lead:', error);
      // В случае ошибки все равно переходим на страницу завершения
      router.push('/complete');
    } finally {
  ...
  return (
    <section className="relative bg-gradient-to-r from-[#ff6a2b] to-[#a84fd4] min-h-[520px] flex items-center overflow-hidden">
      <div className="container mx-auto px-4 md:px-8 relative z-20 flex flex-col items-center justify-center w-full">
            ...
            <div className="relative z-10 bg-white rounded-[32px] shadow-2xl flex flex-col md:flex-row items-stretch pt-10 pb-10 px-6 gap-0 md:gap-0 w-full max-w-full">
              <div className="flex-1 flex flex-col justify-center pr-0 md:pr-8">
                ...
                <div className="flex items-center mb-6">
                  <svg className="w-6 h-6 text-gray-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 12.414A6 6 0 1112.414 13.414l4.243 4.243a1 1 0 001.414-1.414z" /></svg>
                  <span className="text-lg text-gray-700 font-medium">{city && city.trim().length > 0 ? city.trim() : 'в России'}</span>
                </div>
                <SupportOnlyBlock>
              ...
              <div className="flex flex-col justify-center items-center md:items-start md:pl-8 mt-8 md:mt-0 md:w-[300px] border-t md:border-t-0 md:border-l border-gray-100">
                {isSupportOnly ? (
                  ...
                  <>
                    ...
                    >
                      <svg className="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" /></svg>
                      Заказать звонок
                    </button>
                  </>
                )}
        ...
        {/* Блок преимуществ (оставить как есть) */}
